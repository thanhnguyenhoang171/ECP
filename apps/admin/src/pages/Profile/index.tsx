...
const Profile: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 font-sans text-slate-600">
      <div className="max-w-5xl mx-auto">
        {/* Top Navigation */}
        <div className="mb-8 flex items-center justify-between">
          <BackButton label="Quay lại Dashboard" destination="/dashboard" />
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Hồ sơ cá nhân</span>
        ...
        <Row gutter={[24, 24]}>
          ...
          <Col xs={24} md={8}>
            <Card className="text-center h-full overflow-hidden">
              <div className="mb-8 relative inline-block group">
                {/* Avatar Container */}
                <div className="relative w-[150px] h-[150px] rounded-full ring-4 ring-white shadow-soft overflow-hidden cursor-pointer group-hover:ring-primary-100 transition-all duration-300">
                  <AntdAvatar 
                    size={150} 
                    src={avatarUrl}
                    icon={<UserOutlined />} 
          ...
          <Col xs={24} md={16}>
                ...
                <Row gutter={16}>
                  <Col span={12}>
                    <FormControl name="first_name" control={control} label="Họ & Tên đệm" error={errors.first_name?.message} required>
                      <Input placeholder="Nhập họ" className="h-10 border-slate-200" />
                    </FormControl>
                  </Col>
                  <Col span={12}>
                    <FormControl name="last_name" control={control} label="Tên" error={errors.last_name?.message} required>
                      <Input placeholder="Nhập tên" className="h-10 border-slate-200" />
                    </FormControl>
                  </Col>
                </Row>

                <FormControl name="email" control={control} label="Địa chỉ Email" error={errors.email?.message} required>
                  <Input prefix={<MailOutlined className="text-slate-400 mr-2" />} placeholder="email@example.com" className="h-10 border-slate-200" />
                </FormControl>

                <Row gutter={16}>
                  <Col span={12}>
                    <FormControl name="phone_number" control={control} label="Số điện thoại" error={errors.phone_number?.message} required>
                      <Input prefix={<PhoneOutlined className="text-slate-400 mr-2" />} placeholder="0123 456 789" className="h-10 border-slate-200" />
                    </FormControl>
                  </Col>
                  <Col span={12}>
                    <FormControl name="username" control={control} label="Tên đăng nhập" error={errors.username?.message} required>
                      <Input placeholder="username" disabled className="h-10 bg-slate-50 border-slate-200 text-slate-400" />
                    </FormControl>
                  </Col>
                </Row>

                <div className="flex justify-end mt-4">
                  <Button type="primary" htmlType="submit" icon={<SaveOutlined />} className="px-8 h-10 font-bold shadow-soft">
                    Lưu thay đổi
                  </Button>
                </div>
              </Form>
            ...
            <Card className="mt-6 border-amber-100 bg-amber-50/20">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-slate-800 mb-1 flex items-center gap-2"><KeyOutlined className="text-amber-500" /> Bảo mật & Mật khẩu</h4>
                  <p className="text-xs text-slate-500 m-0">Bạn nên thay đổi mật khẩu định kỳ để bảo vệ tài khoản.</p>
                </div>
                <Button className="border-amber-200 text-amber-600 hover:text-amber-700 hover:border-amber-300 font-semibold bg-white flex items-center justify-center">
                  Đổi mật khẩu
                </Button>
              </div>
            </Card>
